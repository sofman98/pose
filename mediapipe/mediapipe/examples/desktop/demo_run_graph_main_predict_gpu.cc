...
//For sending values
#define SERVER_PORT htons(50888)
#define FCOUNT 42
...
::mediapipe::Status RunMPPGraph() {
  ...
  int n ,i;
  float arr[FCOUNT];
  int bufLen = FCOUNT * sizeof(float);
  char buffer[bufLen];
  bool send_row;
  ...
  setsockopt(serverSock, SOL_SOCKET, SO_REUSEPORT, &reusePort, sizeof(reusePort));
      cout << "Server sock  " << serverSock << endl;
  ...
  while (grab_frames) {
    ...
    for (const ::mediapipe::NormalizedLandmark& landmark : output_landmarks) {
  	...
  	if(send_row){
        memcpy(buffer, arr, bufLen);
        n = write(clientSock, buffer, bufLen);
    }
